...
class ActionParser {
	...
	private static parseActionHeader(dataView: ProgressiveDataView): KeyboardEvent | undefined {
	...
	private static screenAction(dataView: ProgressiveDataView): ScreenAction {
		const _videoOffset = dataView.getInt32();
		const _videoLength = dataView.getInt32();
		const dataLength = dataView.getInt32();
		const _fileName = dataView.getString(dataLength);
	...
	private static textHighlightAction(dataView: ProgressiveDataView): TextHighlightAction {
		...
		return new TextHighlightAction(handle, color, textBounds);
	}
